...
class Site:
    ...
    def update_news(self):
        if self.opcao.lower()=='globo':
            ...
            tg_class2='post-multicontent__link--content__image'
            news_dict_globo={}
            for noticia in noticias:
                if noticia.h2 != None:
                    if tg_class1 in noticia.h2.get('class') or  tg_class1 in noticia.h2.get('class'):
                     news_dict_globo[noticia.h3.text]=noticia.get('href')
                    
    
            self.news=news_dict_globo

        if self.opcao.lower()=='bolavip':
            url='https://br.bolavip.com/'
            browsers = {'User-Agent': "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 \(KHTML, like Gecko) Chrome / 86.0.4240.198Safari / 537.36"}
            page = requests.get(url, headers = browsers)
            resposta=page.text
            soup=BeautifulSoup(resposta,'html.parser')
            noticias =soup.find_all('a')
            class1="card-title__link"
            news_dict_bolaVip={}
            
            for noticia in noticias:
                if class1 in noticia.get('class'):

                    noticia_title= noticia.get('title')
                    news_dict_bolaVip[noticia_title]=noticia.get('href')
                   
                
            self.news=news_dict_bolaVip
